(function(){
  ...
  function serveStaticAssets(app, opts) {
    ...
    // Connect-inject requires request header Accept: text/html.
    // By doing this we ensure that it's always set for all clients requesting
    // files ending with .html
    function hackConnectInject(req, res, next) {
      var url = req.originalUrl;
      if(url == '/' || (_.endsWith(url, '.html') && url.indexOf('/connect') === -1)) {
        req.headers.accept = 'text/html';
      }
      next();
    }

    app.use('/', hackConnectInject, connectInject({
      snippet: snippet
